...
module.exports = class TaskQueue {
    ...
    async _run() {
        ...
        if (this._pausedCount > 0 || this._paused) {
            await Promise.all([
                new Promise(resolve => this._resolvePausedState = resolve),
                new Promise(resolve => this._resoveAccumulatedPausedState = resolve),
            ])
        }
     ...
     */
    pause(accumulated = true) {
        accumulated ? this._pausedCount++ : (this._paused = true);
    }
     ...
     */
    resume(accumulated = true) {
        if (accumulated) {
            this._pausedCount > 0 && this._pausedCount--;
        } else {
            this._paused = false;
        }
        if (this._pausedCount <= 0 && this._resoveAccumulatedPausedState) {
            this._resoveAccumulatedPausedState();
            delete this._resoveAccumulatedPausedState;
        }
        if (!this._paused && this._resolvePausedState) {
